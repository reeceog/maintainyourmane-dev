...
class PluginUpdateChecker_2_3 {
	...
	/**
	 * Rename the update directory to Booking the existing plugin directory.
	 *
	...
	function fixDirectoryName($source, $remoteSource, $upgrader) {
		...
		//Rename the source to Booking the existing plugin directory.
		...
		if ( $source !== $correctedSource ) {
			...
			if ( $wp_filesystem->move($source, $correctedSource, true) ) {
				...
				return new WP_Error(
					'puc-rename-failed',
					'Unable to rename the update to Booking the existing plugin directory.'
				);
			}
		}
