;(function($, window, document, undefined) {
	...
	$win.on('load', function() {
		...
		$('body').on('click','form.booked-form input[type=submit]',function(e){
			...
			var customerType 		= $('#newMatchForm input[name=customer_type]:checked').val(),
				customerID			= $('#newMatchForm select[name=user_id]').val(),
				firstName			= $('#newMatchForm input[name=first_name]').val(),
				firstNameDefault	= $('#newMatchForm input[name=first_name]').attr('title'),
				lastName			= $('#newMatchForm input[name=last_name]').val(),
				email				= $('#newMatchForm input[name=email]').val(),
				emailDefault		= $('#newMatchForm input[name=email]').attr('title'),
				phone				= $('#newMatchForm input[name=phone]').val(),
				mobile				= $('#newMatchForm input[name=mobile]').val(),
				calendar_id			= $('#newMatchForm').attr('data-calendar-id'),
				...
				ajaxRequests.push = $.ajaxQueue({
					...
					success: function(date) {
						...
						$('tr.entryBlock').find('td').load(booked_ajaxURL, {'load':'calendar_date','date':date,'calendar_id':calendar_id},function(){
							...
							$('.bookedMatchTab.active').fadeIn(300);
						});
				...
				ajaxRequests.push = $.ajaxQueue({
					...
					success: function(data) {
						...
						if (data[0] != 'success'){
							...
							// There was an error!
							$thisButton.val('Create Booking').attr('disabled',false);
		...
		// Delete Booking Click from Calendar
		...
		$('.booked-admin-calendar-wrap').on('click', '#bookedMatchTabs li a', function(e) {
			...
			$('#bookedMatchTabs li').removeClass('active');
			$('.bookedMatchTab').hide();
			$('.bookedMatchTab').removeClass('active');

			$thisTab.parent().addClass('active');

			$('#bookedCalendarMatchesTab-'+tabName).fadeIn(100);
			$('#bookedCalendarMatchesTab-'+tabName).addClass('active');
		...
		// Delete Booking from Pending List
		...
		// Approve Booking from Pending List
