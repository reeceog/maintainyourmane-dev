	...
	*/
	if (isset($_POST['load']) && $_POST['load'] == 'new_appointment_form' && isset($_POST['date']) && isset($_POST['timeslot']))
	{
		...
		?><div class="booked-form">
			...
			<p><small><?php _e('Request Booking','booked'); ?></small></p>
	
			<?php $reached_limit = false;
				
			if (is_user_logged_in()):
			
				?><form action="" method="post" id="newMatchForm" data-calendar-id="<?php echo $calendar_id ? $calendar_id : 0; ?>">
					...
					if (!$reached_limit):
						...
						?><p><?php echo sprintf( __( 'You are about to request a Booking for %s.','booked' ), booked_get_name( $current_user->ID )); ?> <?php _e('Please confirm that you would like to request the following Booking:','booked'); ?></p>
						...
						<div class="field">
							<?php if (!$reached_limit): ?>
								<input type="submit" id="submit-request-appointment" class="button button-primary" value="<?php _e('Request Booking','booked'); ?>">
