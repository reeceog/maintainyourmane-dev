...
function booked_render_single_timeslot_form($timeslot_intervals,$type = false){
	...
	?><span class="allday-wrap"><input type="checkbox" id="all_day<?php echo $type ? '_'.$type : ''; ?>" name="all_day" value="1"> <label for="all_day<?php echo $type ? '_'.$type : ''; ?>"><?php _e('Day Booking','booked'); ?></label></span><?php
				...
				<?php foreach($day_loop_english_array as $day):
					...
					echo '<td class="dayTimeslots" data-day="'.$day.'">';
						if (!empty($booked_defaults[$day])):
							...
							echo '<p><small>'.__('No Bookings.','booked').'</small></p>';
						endif;
					echo '</td>';
...
function booked_render_timeslot_info($time_format,$time,$count){
	...
	$html = '<span class="timeslot" data-timeslot="'.$time.'">';
		...
		if ($time[0] == '0000' && $time[1] == '2400'):
			$timeslotText = '<span class="start" style="color:#aaa">' . strtoupper(__('Day Booking','booked')) . '</span>';
		else :
...
function booked_render_count_select($select_name,$placeholder){
	echo '<select name="'.$select_name.'">'; ?>
	<option value="1" selected><?php _e('1 Booking','booked'); ?></option>
	<option value="2"><?php _e('2 Bookings','booked'); ?></option>
	<option value="3"><?php _e('3 Bookings','booked'); ?></option>
	<option value="4"><?php _e('4 Bookings','booked'); ?></option>
	<option value="5"><?php _e('5 Bookings','booked'); ?></option>
	<option value="6"><?php _e('6 Bookings','booked'); ?></option>
	<option value="7"><?php _e('7 Bookings','booked'); ?></option>
	<option value="8"><?php _e('8 Bookings','booked'); ?></option>
	<option value="9"><?php _e('9 Bookings','booked'); ?></option>
	<option value="10"><?php _e('10 Bookings','booked'); ?></option>
	<option value="11"><?php _e('11 Bookings','booked'); ?></option>
	<option value="12"><?php _e('12 Bookings','booked'); ?></option>
	<option value="13"><?php _e('13 Bookings','booked'); ?></option>
	<option value="14"><?php _e('14 Bookings','booked'); ?></option>
	<option value="15"><?php _e('15 Bookings','booked'); ?></option>
	<option value="16"><?php _e('16 Bookings','booked'); ?></option>
	<option value="17"><?php _e('17 Bookings','booked'); ?></option>
	<option value="18"><?php _e('18 Bookings','booked'); ?></option>
	<option value="19"><?php _e('19 Bookings','booked'); ?></option>
	<option value="20"><?php _e('20 Bookings','booked'); ?></option>
	<?php echo '</select>';
}
...
function booked_admin_calendar($year = false,$month = false,$calendar_id = false,$size = false){
	...
	$bookedMatches = new WP_Query($args);
	if($bookedMatches->have_posts()):
		while ($bookedMatches->have_posts()):
			$bookedMatches->the_post();
			global $post;
		...
		<tbody><?php
			...
			for($i = 1; $i <= $rows; $i++){
				...
				for($x = 1; $x <= 7; $x++){
					...
					$day_of_week = date('N',strtotime($check_year.'-'.$check_month.'-'.$date));
	...
	echo '<div class="booked-appt-list">';
		...
		*/
		...
		if ($tabbed):
			...
			?><ul id="bookedMatchTabs" class="bookedClearFix">
			...
			$tab_title = __('Bookings for','booked');
			if (booked_user_role() != 'booked_booking_agent'):
				?><div id="bookedCalendarMatchesTab-default" class="bookedMatchTab<?php if (!$calendar_id): ?> active<?php endif; ?>"><?php
					booked_admin_calendar_date_loop($date,$time_format,$date_format,false,$tab_title,$tabbed,$calendars);
				?></div><?php
			endif;
			foreach($calendars as $calendar):
				?><div id="bookedCalendarMatchesTab-<?php echo $calendar->term_id; ?>" class="bookedMatchTab<?php if ($calendar_id == $calendar->term_id): ?> active<?php endif; ?>"><?php
...
function booked_admin_calendar_date_loop($date,$time_format,$date_format,$calendar_id = false,$tab_title,$tabbed = false,$calendars = false){
	...
	if ($todays_defaults){
		...
		foreach($todays_defaults as $timeslot => $count):
			...
			*/
			...
			if ($timeslot_parts[0] == '0000' && $timeslot_parts[1] == '2400'):
				$timeslotText = __('Day Booking','booked');
			else :
			...
			echo '<div class="timeslot bookedClearFix">';
				...
				echo '<span class="timeslot-count">';
					...
					echo '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">'.$spots_available.' '._n('Booking','Bookings',$spots_available,'booked').' '.__('available','booked').'</span>';
					...
					*/
					...
					if (!empty($appts_in_this_timeslot)):
						...
						echo '<strong>'.$booked_appts.' '._n('Booking','Bookings',$booked_appts,'booked').':</strong>';
				...
				echo '</span>';
				echo '<span class="timeslot-people"><button'.(!$available ? ' disabled' : '').' data-timeslot="'.$timeslot.'" data-date="'.$date.'"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').' class="new-appt button"'.(!$spots_available ? ' disabled' : '').'>'.__('New Booking','booked').' ...</button></span>';
			echo '</div>';
		...
		*/
		...
		if (!empty($appointments_array)):
			...
			echo '<br><p>'.__('There are additional Bookings booked from previously available Bookings:','booked').'</p>';
	...
	*/
		...
		echo '<span class="additional-timeslots">';
		echo '<p>'.__('There are no Bookings available for this day, however there are Bookings booked from previously available Bookings:','booked').'</p>';
		foreach($appointments_array as $appointment):
	...
	*/
		...
		echo '<p>'.__('There are no Bookings available for this day.','booked').' <a href="'.get_admin_url(null,'admin.php?page=booked-settings#defaults').'">'.__('Would you like to add some?','booked').'</a></p>';
	}
}
