		...
		if (is_user_logged_in()):
			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="booked-profile-page" class="booked-shortcode">'; endif;
				...
				echo '<div class="booked-profile-appt-list">';
					...
					if ($historic):
						echo '<h4><span class="count">' . number_format($total_appts) . '</span> ' . _n('Past Booking','Past Bookings',$total_appts,'booked') . '</h4>';
					else:
						echo '<h4><span class="count">' . number_format($total_appts) . '</span> ' . _n('Upcoming Booking','Upcoming Bookings',$total_appts,'booked') . '</h4>';
					endif;
							...
							if (!$historic):
								if ($appt_date_time >= $date_to_compare):
									echo '<div class="booked-cal-buttons">';
										if (!get_option('booked_hide_google_link',false)): echo '<a href="//www.google.com/calendar/render?action=TEMPLATE&text='.urlencode(sprintf(__('Booking with %s','booked'),get_bloginfo('name'))).'&dates='.$google_date_startend.'T'.$google_time_start.'00/'.$google_date_startend_end.'T'.$google_time_end.'00&details=&location=&sf=true&output=xml" target="_blank" rel="nofollow" class="google-cal-button"><i class="fa fa-plus"></i>&nbsp;&nbsp;'.__('Google Calendar','booked').'</a>'; endif;
