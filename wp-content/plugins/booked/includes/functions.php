		...
		<tbody><?php
			...
			for($i = 1; $i <= $rows; $i++){
				...
				for($x = 1; $x <= 7; $x++){
					...
					/* Check to see if the last appointment Booking is passed */
					...
					/* END - Check to see if the last appointment Booking is passed */
					...
					if ($hide_weekends && $day_of_week == 6 || $hide_weekends && $day_of_week == 7):
						...
						$html .= '<span class="date'.($appointments_left && !in_array('prev-date',$classes) && !in_array('blur',$classes) ? ' tooltipster" title="'.sprintf( _n('%d Booking','%d Bookings',$appointments_left,'booked'), $appointments_left) : '').'"><span class="number">'. $date .'</span></span>';
						$html .= '</td>';
...
function booked_fe_calendar_date_content($date,$calendar_id = false){
	...
	echo '<div class="booked-appt-list">';
		...
		*/
		...
		$bookedMatches = new WP_Query( apply_filters('booked_fe_date_content_query',$args) );
		if($bookedMatches->have_posts()):
			while ($bookedMatches->have_posts()):
				$bookedMatches->the_post();
				global $post;
		...
		*/
		...
		echo '<h2>'.__('Available Bookings','booked').':</h2>';
		...
		*/
		...
		if ($todays_defaults){
				...
				*/
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$button_text = (!$spots_available || !$available ? __('Unavailable','booked') : __('Book Booking','booked'));
						$html = '<div class="timeslot bookedClearFix">';
							...
							$html .= '<span class="timeslot-count">';
								$html .= '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">'.$spots_available.' '._n('Booking','Bookings',$spots_available,'booked').' '.__('available','booked').'</span>';
							$html .= '</span>';
			...
			if (!$temp_count):
				...
				echo '<p>'.__('There are no appointment Bookings available for this day.','booked').'</p>';
		...
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'update-user' ) {
		    ...
		    if (isset($_POST['pass1']) && isset($_POST['pass2']) && $_POST['pass1'] && $_POST['pass2'] ) {
		        if ( $_POST['pass1'] == $_POST['pass2'] )
		            ...
		            $error[] = __('The passwords you entered do not Booking.  Your password was not updated.', 'profile');
		    }
