...
/*
Plugin Name: Booked
Plugin URI: http://buoy.digital
Description: Booking System.
Version: 1.6.20
Author: Buoy
Author URI: https://buoy.digital
*/

define('BOOKED_VERSION', '1.6.20');
define('BOOKED_FA_VERSION','4.5.0');
	...
	class booked_plugin {
		...
		**********************/
		...
		public function add_menu() {
			add_menu_page( __('Bookings','booked'), __('Bookings','booked'), 'edit_booked_appointments', 'booked-appointments', array(&$this, 'admin_calendar'), 'dashicons-calendar-alt', 58 );
		...
		// Move Taxonomy (custom calendars) to Bookings Menu
			...
			if (current_user_can('edit_booked_appointments')):
				...
				if ($pending && $page != 'booked-pending'):
					...
					echo '<div class="update-nag">';
						echo sprintf( _n( 'There is %s pending appointment.', 'There are %s pending Bookings.', $pending, 'booked' ), $pending ).' <a href="'.get_admin_url().'admin.php?page=booked-pending">'._n('View Pending Booking','View Pending Bookings',$pending,'booked').'</a>';
					echo '</div>';
		...
		**********************/
		...
		public function booked_add_user_columns( $defaults ) {
			$defaults['booked_appointments'] = __('Bookings', 'booked');
			return $defaults;
		}
		public function booked_add_custom_user_columns($value, $column_name, $id) {
			if( $column_name == 'booked_appointments' ) {
				...
				if ($count){
					...
					echo '<strong>'.$count.' '._n('Upcoming Booking','Upcoming Bookings',$count,'booked').':</strong>';
			...
			if (in_array($current_page,$this->booked_screens) || !empty($screen) && $screen->id == 'dashboard'): ?>
				...
				<script type="text/javascript"><?php
					...
					var i18n_slot 						= '<?php echo esc_js(__('slot','booked')); ?>',
						...
						i18n_bulk_add_confirm			= '<?php echo esc_js(__('Are you sure you want to add those bulk Bookings?','booked')); ?>',
						i18n_all_fields_required		= '<?php echo esc_js(__('All fields are required.','booked')); ?>',
						i18n_single_add_confirm			= '<?php echo esc_js(__('You are about to add the following Bookings','booked')); ?>',
						i18n_to							= '<?php echo esc_js(__('to','booked')); ?>',
						i18n_all_day					= '<?php echo esc_js(__('Day Booking','booked')); ?>',
						...
						i18n_confirm_ts_delete			= '<?php echo esc_js(__('Are you sure you want to delete this Booking?','booked')); ?>',
						i18n_confirm_cts_delete			= '<?php echo esc_js(__('Are you sure you want to delete this custom Booking?','booked')); ?>',
						i18n_confirm_appt_delete		= '<?php echo esc_js(__('Are you sure you want to cancel this Booking?','booked')); ?>',
						i18n_appt_required_fields		= '<?php echo esc_js(__('A first name and an email address are required fields.','booked')); ?>',
						i18n_confirm_appt_approve		= '<?php echo esc_js(__('Are you sure you want to approve this Booking?','booked')); ?>';
		...
		public function inline_scripts() { ?>
			...
			<script type="text/javascript"><?php
				...
				var i18n_confirm_appt_delete 		= '<?php echo esc_js(__('Are you sure you want to cancel this appointment?','booked')); ?>',
					...
					i18n_request_appointment		= '<?php echo esc_js(__('Request Booking','booked')); ?>',
					i18n_fill_out_required_fields 	= '<?php echo esc_js(__('Please fill out all required fields.','booked')); ?>',
					i18n_appt_required_fields		= '<?php echo esc_js(__('A first name and an email address are required fields.','booked')); ?>';
